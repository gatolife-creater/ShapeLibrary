...
function mouseWheel(event) {
    var center = new Point(mouseX - width / 2, mouseY - height / 2);
    if (event.deltaY > 0) {
        var scope = 1.015;
    } else {
        var scope = 0.985;
    ...
    newLinear = newLinear.magnify(center, scope);
    newQuadratic = newQuadratic.magnify(center, scope);
    newTri = newTri.magnify(center, scope);
    newQuad = newQuad.magnify(center, scope);
    newPolygon = newPolygon.magnify(center, scope);
    newL = newL.magnify(center, scope);
    newP = newP.magnify(center, scope);
}
